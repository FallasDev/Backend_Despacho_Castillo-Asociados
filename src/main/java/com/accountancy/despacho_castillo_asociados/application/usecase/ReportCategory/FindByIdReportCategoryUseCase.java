...
public class FindByIdReportCategoryUseCase {
    ...
    public DomainReportCategory execute(int id) {
        DomainReportCategory domainReportCategory = reportCategoryRepository.findById(id).orElse(null);

        if (domainReportCategory == null || !domainReportCategory.isActive()) {
